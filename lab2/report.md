# Лабораторная работа №2

**University:** [ITMO University](https://itmo.ru/ru/)  
**Faculty:** [FICT](https://fict.itmo.ru)  
**Course:** [Introduction in Web Technologies](https://itmo-ict-faculty.github.io/introduction-in-web-tech/)  
**Year:** 2025  
**Group:** U4225  
**Author:** Корепанов Андрей    
**Lab:** Lab2   
**Date of create:** 23.11.2025  
**Date of finished:** 24.11.2025  

## Ход работы
1. Создание сервиса на основе готового образа и с минимальными настройками (128mb RAM, 1 core)
![alt text](heten.png)

2. Рабочий сервис по ссылке
![alt text](<Pasted Graphic 9.png>)

3. Анализ логов и метрик
    - Количество запросов и время ождиания ответа![alt text](<Pasted Graphic 10.png>)
    - Время ответа полного цикла и описание каждого этапа запроса ![alt text](<Pasted Graphic 11.png>)
    - количество контейнеров ![alt text](<Pasted Graphic 12.png>)
    - Потребление CPU и RAM ![alt text](<Pasted Graphic 13.png>)
    - Отправлено/Получено байт ![alt text](<Pasted Graphic 14.png>)
    - Максимальное количество параллельных запросов и время затраченное на поднятие контейнера ![alt text](<Pasted Graphic 15.png>)
    - Логи запросов ![alt text](<Pasted Graphic 16.png>)

4. Добавление новой ревизии с новым портом 8090
![alt text](<Pasted Graphic 17.png>)

5. Установка новой ревизии
![alt text](<Pasted Graphic 19.png>)

6. Результат по ссылке (ревизия изменилась)
![alt text](<Pasted Graphic 18.png>)

7. Установка старой ревизии
![alt text](<Pasted Graphic 20.png>)

8. Результат по ссылке (ревизия изменилась)
![alt text](<Pasted Graphic 21.png>)

9. Удаление всех лишних сервисов
![alt text](<Pasted Graphic 22.png>)